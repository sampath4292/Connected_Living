...
export default function SecurityScannerPage() {
    ...
    const checkBlacklist = async (name: string): Promise<{ isBlacklisted: boolean; reason?: string }> => {
        const blacklist = await api.getBlacklist()
        const found = blacklist.find(b => b.name.toLowerCase() === name.toLowerCase())
        if (found) {
            return { isBlacklisted: true, reason: found.reason }
        }
        return { isBlacklisted: false }
    }

    const handleQrScan = useCallback(async (data: string | null) => {
        if (!data) return
