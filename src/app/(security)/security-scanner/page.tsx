...
export default function SecurityScannerPage() {
    ...
    // Mock blacklist check since it's not in the API yet
    const checkBlacklist = async (name: string) => {
        // Common malicious/blacklisted names for demo
        const blacklisted = ["Spammer", "Trespasser", "Unknown Suspicious"]
        if (blacklisted.includes(name)) {
            return { isBlacklisted: true, reason: "Security violation history" }
        }
    ...
    const handleManualSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const result = await api.security.scanAny({ code })
    ...
    const scanResultOverlay = scanStatus !== "idle" && (
        <div className="absolute inset-0 z-30 bg-black/80 backdrop-blur-sm flex items-center justify-center p-6 animate-in fade-in duration-200 h-full w-full">
            <div className={cn(
                "w-full max-w-sm bg-card text-white rounded-3xl p-6 shadow-2xl border flex flex-col items-center text-center gap-4 text-foreground",
                scanStatus === "success" ? "border-green-500/20" : "border-red-500/20"
            )}>
                <div className={cn(
                ...
                {scanStatus === "loading" && (
                    <>
                        <h2 className="text-xl font-bold">Verifying Code</h2>
                        <p className="text-muted-foreground text-sm">Checking database...</p>
                    </>
                )}
                ...
                {scanStatus === "success" && scannedVisitor && (
                    <>
                        ...
                        <div className="flex flex-col gap-1 w-full bg-muted/30 rounded-xl p-4">
                            ...
                            <div className="flex justify-between text-xs text-muted-foreground">
                                <span>Unit</span>
                                <span className="font-mono">{scannedVisitor.unitNumber}</span>
                            </div>
                            <div className="flex justify-between text-xs text-muted-foreground">
                                <span>Type</span>
                                <span className="font-medium">{scannedVisitor.type}</span>
                            </div>
                            <div className="flex justify-between mt-2 pt-2 border-t border-border">
                                <span className="text-xs text-muted-foreground">{scannedVisitor.status === "INSIDE" ? "In Time" : "Out Time"}</span>
                                <span className="text-xs font-mono">
                                    {scannedVisitor.status === "INSIDE"
                                        ? (scannedVisitor.entryTime ? new Date(scannedVisitor.entryTime).toLocaleTimeString() : new Date().toLocaleTimeString())
                                        : (scannedVisitor.exitTime ? new Date(scannedVisitor.exitTime).toLocaleTimeString() : new Date().toLocaleTimeString())}
                                </span>
                            </div>
                        </div>
                    </>
                ...
                {scanStatus === "success" && scannedStaff && (
                    <>
                        <h2 className="text-xl font-bold">{scannedStaff.status === "IN" ? "Staff Check-In" : "Staff Check-Out"}</h2>
                        <div className="flex flex-col gap-1 w-full bg-muted/30 rounded-xl p-4">
                            ...
                            <div className="flex justify-between text-xs text-muted-foreground">
                                <span>Staff ID</span>
                                <span className="font-mono">{scannedStaff.staffId}</span>
                            </div>
                            <div className="flex justify-between mt-2 pt-2 border-t border-border">
                                <span className="text-xs text-muted-foreground">Time</span>
                                <span className="text-xs font-mono">
                                    {scannedStaff.status === "IN"
                                        ? (scannedStaff.checkInAt ? new Date(scannedStaff.checkInAt).toLocaleTimeString() : new Date().toLocaleTimeString())
                                        : (scannedStaff.checkOutAt ? new Date(scannedStaff.checkOutAt).toLocaleTimeString() : new Date().toLocaleTimeString())}
                                </span>
                            </div>
                        </div>
                    </>
        ...
        <>
            ...
            <div className="lg:hidden min-h-screen bg-black text-white flex flex-col relative pb-[env(safe-area-inset-bottom)]">
                ...
                <div className="absolute top-0 left-0 right-0 p-4 z-20 flex justify-between items-start bg-gradient-to-b from-black/80 to-transparent pointer-events-none">
                    <Link href="/security" className="pointer-events-auto p-2 bg-black/40 backdrop-blur-md rounded-full text-white/80 hover:text-white hover:bg-black/60 transition-all">
                        <ArrowLeft size={24} />
                    </Link>
