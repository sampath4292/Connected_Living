...
const MOCK_SAVED_VISITORS: SavedVisitorItem[] = [
  {
    ...
    name: "Mohan",
    type: "Guest",
    relation: "Tution Teacher",
    phone: "9876543210",
    email: "mohan@tutor.com",
...
const MOCK_FREQUENT_VISITORS: FrequentVisitorItem[] = [
  {
    ...
    allowedTimeSlot: "Morning (8am-12pm)",
    isActive: true,
    avatar: "S",
  },
  {
    ...
    allowedTimeSlot: "Afternoon (2pm-4pm)",
    isActive: true,
  },
];
...
const MOCK_BLACKLIST: BlacklistItem[] = [
  {
    id: "BL-1",
    name: "Blacklist User",
    reason: "Repeated disturbance and abuse to staff.",
    date: "2024-01-15",
    reportedBy: "Ramesh Guard",
  },
  {
    id: "BL-2",
    name: "Unknown Vendor",
    reason: "Tried to enter without valid approval multiple times.",
    date: "2024-02-01",
    reportedBy: "Suresh Supervisor",
  },
];
...
const MOCK_VEHICLE_ENTRIES: VehicleEntryItem[] = [
  ...
  {
    id: "VE-8",
    vehicleNumber: "KA-08-OP-3456",
    type: "Car",
    ownerName: "Swiggy Delivery",
    unitId: "C-202",
    status: "Exited",
    entryTime: "1:15 PM",
    exitTime: "1:30 PM",
  ...
  {
    id: "VE-10",
    vehicleNumber: "KA-10-ST-1234",
    type: "Truck",
    ownerName: "Furniture Delivery",
    unitId: "B-101",
    status: "Exited",
    entryTime: "10:00 AM",
    exitTime: "11:45 AM",
...
function transformBooking(booking: Booking): BookingItem {
  ...
  const bookingDate = firstSlot
    ? new Date(firstSlot).toLocaleDateString()
...
function transformRecurringStaff(staff: RecurringStaff): FrequentVisitorItem {
  return {
    ...
    validUntil: staff.validTo
      ? new Date(staff.validTo).toISOString().split("T")[0]
      : "",
...
function transformVisitorGroup(group: VisitorGroup): VisitorItem {
  ...
  const latestLog = group.entryLogs?.slice().sort((a, b) => {
...
export const api = {
    ...
    try {
      ...
      const target =
        ...
        (typeof window !== "undefined"
          ? sessionStorage.getItem("otp_target")
          : null) ||
        "";
      const response = await httpClient.post<AuthResponse>(
        "/auth/otp/verify",
        { identifier: target, otp },
        { skipAuth: true },
      );
      tokenManager.setTokens(response.accessToken, response.refreshToken);
      return true;
  ...
  getFamilyMembers: async (): Promise<FamilyMemberItem[]> => {
    ...
    try {
      const data = await httpClient.get<{ members: FamilyMemberItem[] }>(
        "/residents/me/family",
      );
      return data.members || [];
    } catch {
  ...
  addFamilyMember: async (
    member: Omit<FamilyMemberItem, "id">,
  ...
  updateFamilyMember: async (
    id: string,
    data: Partial<FamilyMemberItem>,
  ): Promise<FamilyMemberItem | undefined> => {
    try {
      return await httpClient.patch<FamilyMemberItem>(
        `/residents/me/family/${id}`,
        data,
      );
    } catch {
      return undefined;
  ...
  addVehicle: async (
    vehicle: Omit<VehicleItem, "id" | "userId">,
  ): Promise<VehicleItem> => {
  ...
  updateVehicle: async (
    id: string,
    data: Partial<VehicleItem>,
  ): Promise<VehicleItem | undefined> => {
    try {
  ...
  inviteVisitor: async (
    ...
    try {
      ...
      if (data.type === "Guest") {
        visitors = data.guests.map((g) => ({
          name: g.name,
          mobileNumber: g.phone,
        }));
        visitorTime = data.time;
      } else if (data.type === "Delivery") {
        visitors = [
          { name: data.name || data.vendor, mobileNumber: data.phone },
        ];
      ...
      return {
        ...
        message:
          data.type === "Guest"
            ...
            : "Details shared with Security",
      };
    } catch (e) {
      return {
        success: false,
        message: e instanceof Error ? e.message : "Failed to create invitation",
      };
    }
  },
  ...
  getSavedVisitorById: async (
    _id: string,
  ): Promise<SavedVisitorItem | undefined> => {
    void _id;
    return undefined;
  },

  updateSavedVisitor: async (
    _id: string,
    _data: Partial<SavedVisitorItem>,
  ): Promise<SavedVisitorItem | undefined> => {
    void _id;
    void _data;
    return undefined;
  ...
  getFrequentVisitors: async (): Promise<FrequentVisitorItem[]> => {
    try {
      const staff = await httpClient.get<RecurringStaff[]>(
        "/visitors/recurring",
      );
  ...
  getFrequentVisitorById: async (
    id: string,
  ): Promise<FrequentVisitorItem | undefined> => {
    try {
      const staff = await httpClient.get<RecurringStaff>(
        `/visitors/recurring/${id}`,
      );
  ...
  addFrequentVisitor: async (
    visitor: Omit<FrequentVisitorItem, "id">,
  ): Promise<RecurringStaff | null> => {
    try {
      ...
      const validTo = visitor.validUntil
        ? new Date(visitor.validUntil).toISOString()
      ...
      const created = await httpClient.post<RecurringStaff>(
        "/visitors/recurring",
        request,
      );
      return created;
  ...
  updateFrequentVisitor: async (
    id: string,
    updates: Partial<FrequentVisitorItem>,
  ): Promise<boolean> => {
    try {
      await httpClient.patch(`/visitors/recurring/${id}`, {
        name: updates.name,
        mobileNumber: updates.mobile,
        validTo: updates.validUntil
  ...
  getVisitorAttendance: async (id: string): Promise<AttendanceItem[]> => {
    try {
      const logs = await httpClient.get<StaffAttendanceLog[]>(
        `/visitors/recurring/${id}/attendance`,
      );
      ...
      return logs.map((log) => ({
        ...
        checkIn: new Date(log.checkInAt).toLocaleTimeString([], {
          hour: "2-digit",
          minute: "2-digit",
        }),
        checkOut: log.checkOutAt
          ? new Date(log.checkOutAt).toLocaleTimeString([], {
              hour: "2-digit",
              minute: "2-digit",
            })
          : undefined,
  ...
  getAmenitySlots: async (amenityId: string): Promise<AmenitySlot[]> => {
    try {
      return await httpClient.get<AmenitySlot[]>(
  ...
  getServiceRequests: async (): Promise<ServiceRequestItem[]> => {
    try {
      const requests =
        await httpClient.get<ServiceRequest[]>("/service-requests");
  ...
  getServiceRequestById: async (
    id: string,
  ): Promise<ServiceRequestItem | undefined> => {
    try {
      const requests =
        await httpClient.get<ServiceRequest[]>("/service-requests");
  ...
  createServiceRequest: async (
    request: Omit<CreateServiceRequestRequest, "unitId">,
    ...
    const created = await httpClient.post<ServiceRequest>(
      "/service-requests",
      payload,
    );
  ...
  createPaymentIntent: async (
    ...
    return httpClient.post<PaymentIntentResponse>("/payments/intent", {
      invoiceIds,
    });
  },
  ...
  getActivities: async (): Promise<ActivityItem[]> => {
    try {
      const [bookings, requests, visitorGroups, recurringVisitors] =
        await Promise.all([
          httpClient
            .get<Booking[]>("/amenities/bookings", { cache: "no-store" })
            .then((res) => res.map(transformBooking))
            .catch(() => []),
          httpClient
            .get<ServiceRequest[]>("/service-requests", { cache: "no-store" })
            .then((res) => res.map(transformServiceRequest))
            .catch(() => []),
          httpClient
            .get<VisitorGroup[]>("/visitors/groups", { cache: "no-store" })
            .catch(() => []),
          httpClient
            .get<RecurringStaff[]>("/visitors/recurring", { cache: "no-store" })
            .catch(() => []),
        ]);
  ...
  getNotifications: async (): Promise<NotificationItem[]> => {
    try {
      const notifications =
        await httpClient.get<Notification[]>("/notifications");
  ...
  getNotices: async (): Promise<NoticeItem[]> => {
    try {
      const announcements =
        await httpClient.get<Announcement[]>("/announcements");
  ...
  getCommunityMessages: async (): Promise<CommunityMessageItem[]> => {
    try {
      ...
      const messages = await httpClient.get<ChatMessage[]>(
        `/chat/groups/${groups[0].id}/messages`,
      );
  ...
  sendCommunityMessage: async (text: string): Promise<CommunityMessageItem> => {
    ...
    const msg = await httpClient.post<ChatMessage>(
      `/chat/groups/${groups[0].id}/messages`,
      { content: text },
    );
    return {
  ...
  getCommunityEventById: async (
    _id: number,
  ): Promise<CommunityEventItem | undefined> => {
    void _id;
    return undefined;
  },
  ...
  security: {
    login: async (
      credentials: SecurityLoginRequest,
    ): Promise<SecurityAuthResponse> => {
      const response = await httpClient.post<SecurityAuthResponse>(
        "/security/auth/login",
        credentials,
        { skipAuth: true },
      );
    ...
    getExpectedVisitors: async (): Promise<SecurityVisitorEntry[]> => {
      try {
        const groups = await httpClient.get<SecurityVisitorGroup[]>(
          "/security/visitors/expected",
        );
    ...
    getInsideVisitors: async (): Promise<SecurityVisitorEntry[]> => {
      try {
        const logs = await httpClient.get<SecurityVisitorLog[]>(
          ...
          .map(
            (log) =>
              ({
                ...
                status:
                  log.status === "ENTERED" && !log.exitAt ? "INSIDE" : "EXITED",
                ...
                gateId: log.gateId,
                approvalType:
                  log.scanMethod === "QR" || log.group?.shortCode
                    ? "Pre-approved"
                    : "Sudden",
              }) as SecurityVisitorEntry,
          )
    ...
    scanVisitor: async (
      data: ScanVisitorRequest,
    ): Promise<SecurityVisitorEntry> => {
      const response = await httpClient.post<SecurityScanVisitorResponse>(
        "/security/visitors/scan",
        data,
      );
      const log = response.log;
      const deniedReason =
        response.warning === "DUPLICATE_SCAN"
          ? "Duplicate scan (already inside)"
          : undefined;
      return {
        id: log.id,
        visitorName: log.group?.visitors?.[0]?.name || "Unknown",
    ...
    scanVisitorCode: async (data: {
      code: string;
      gateId?: string;
    }): Promise<SecurityVisitorEntry> => {
      const response = await httpClient.post<SecurityScanVisitorResponse>(
        "/security/visitors/scan-code",
        data,
      );
      const log = response.log;
      const deniedReason =
    ...
    scanStaff: async (data: {
      qrCodeId: string;
      gateId?: string;
      direction?: "IN" | "OUT";
    }): Promise<SecurityStaffScanEntry> => {
      const response = await httpClient.post<SecurityStaffScanEntry>(
        "/security/staff/scan",
        data,
      );
    ...
    createWalkIn: async (
      data: CreateWalkInRequest,
    ): Promise<{ requestId: string }> => {
      return httpClient.post<{ requestId: string }>(
        "/security/visitors/walkin",
        data,
      );
    },

    getPendingWalkIns: async (): Promise<SecurityPendingWalkIn[]> => {
      return httpClient.get<SecurityPendingWalkIn[]>(
        "/security/visitors/walkin/pending",
      );
    },

    approveWalkInEntry: async (
      requestId: string,
      data: WalkInEntryRequest,
    ): Promise<SecurityVisitorEntry> => {
      return httpClient.post<SecurityVisitorEntry>(
        `/security/visitors/walkin/${requestId}/entry`,
        data,
      );
    },
    ...
    getInsideVehicles: async (): Promise<VehicleEntryItem[]> => {
      try {
        const logs = await httpClient.get<SecurityVehicleLog[]>(
          "/security/vehicles/inside",
        );
        return logs.map((log) => ({
          id: log.id,
    ...
    getVehicleHistory: async (date: string): Promise<VehicleEntryItem[]> => {
      try {
        const logs = await httpClient.get<SecurityVehicleLog[]>(
          `/security/vehicles/history?date=${date}`,
        );
        return logs.map((log) => ({
          id: log.id,
          vehicleNumber: log.vehicleNumber,
    ...
    logVehicleEntry: async (
      data: SecurityVehicleEntryRequest,
    ): Promise<void> => {
    ...
    createEmergencyAlert: async (message: string): Promise<SOSLogItem> => {
      return httpClient.post<SOSLogItem>("/security/emergency-alerts", {
        message,
      });
    },
    ...
    getEmergencyAlerts: async (): Promise<SOSLogItem[]> => {
      try {
        const alerts = await httpClient.get<SecurityEmergencyAlert[]>(
